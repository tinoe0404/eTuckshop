...
export default function OrderDetailClient({ order }: OrderDetailClientProps) {
    ...
    // Fetch QR code separately (only if needed) - Keeping this hook as it might poll or be specific
    ...
    }, [generateQRMutation, orderId, router]);

    const handleCancelOrder = useCallback(() => {
        cancelOrderMutation.mutate(orderId, {
            onSuccess: () => router.refresh()
        });
    }, [cancelOrderMutation, orderId, router]);
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {order.status === 'PENDING' && (
                                            <div className="flex items-start space-x-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-sm font-semibold text-gray-500">Awaiting Pickup</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
